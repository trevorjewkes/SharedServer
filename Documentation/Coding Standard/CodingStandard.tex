...
All code SHALL be formatted with clang format with
  the.clang\_format file found in the repository. \\ \\
\textit
{
Rationale:
  This ensures that all code has similar white spacing and is therefore,
    easy to read.
}
...
\textit
{
Rationale: Consistent naming conventions ensures that proper care is taken for special cases, ie static variables and pointers.
}
\\
...
{
Rationale:
  All caps lets developers know a value is immutable which helps in debugging
    purposes
}
\\
...
\textit
{
Rationale: PascalCase allows for easy identification of user defined types.
}
\\
...
\textit
{
Rationale: camelCase allows for easy identification of user defined types.
}
\\
...
{
Rationale:
  The books listed above are considered by many to be industry standard and give
    a good overview of good practices.
}
...
Functions should be named in a way that they can be easily understood by the
  name
    .Any function with at least one parameter SHALL have a brief comment saying
      what the function does
    .Any part of a block of code that is hard to follow SHOULD be prefaced with
      TRICKY : with an explanation on why the code is that way. \\
\textit
{
Rationale:
  Although aptly named functions are preferred,
    comments help to communicate developer intent.
}
...
All variables SHALL be initialized,
  member variables SHALL be initialized in an initialization list. \\
\textit
{
Rationale:
  Uninitialized variables can lead to unexpected errors that are hard to debug.
}
...
{
Rationale:
  If an exception is ever thrown, poorly managed memory can be leaked causing
                                    the state of a program to become unstable.
}
\\
